...
/// <summary>
/// Componente de inicialización de datos para la base de datos de Pedidos360.
...
public class DataInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        migrateVentaPedidoSchema();
        migrateAuditoriaSchema();
        seedUsuariosSistema();
        seedCategorias();
        seedDirecciones();
    ...
    private void migrateClienteAuthSchema() {
        ...
        jdbcTemplate.update(
            """
            UPDATE cliente
            SET email = CONCAT('cliente', id, '@pedidos360.cl')
    ...
    private void migrateVentaPedidoSchema() {
        Integer estadoCount = jdbcTemplate.queryForObject(
            """
        ...
        if (estadoCount != null && estadoCount == 0) {
            jdbcTemplate.execute("ALTER TABLE venta ADD COLUMN estado VARCHAR(30) NOT NULL DEFAULT 'CREADO'");
        }

        Integer fechaCount = jdbcTemplate.queryForObject(
            """
            SELECT COUNT(*) FROM information_schema.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'venta' AND COLUMN_NAME = 'fecha'
            """,
            Integer.class
        );
        if (fechaCount != null && fechaCount == 0) {
            jdbcTemplate.execute("ALTER TABLE venta ADD COLUMN fecha DATE");
        }

        jdbcTemplate.update(
            """
            UPDATE venta v
            SET v.fecha = COALESCE(
                (SELECT MIN(d.fecha) FROM detalle_venta d WHERE d.venta_id = v.id),
                CURRENT_DATE
            )
            WHERE v.fecha IS NULL
            """
        );
        jdbcTemplate.update("UPDATE venta SET estado = 'CREADO' WHERE estado IS NULL OR estado = ''");
    }

    private void migrateAuditoriaSchema() {
        jdbcTemplate.execute(
            """
    ...
    private void seedUsuariosSistema() {
        jdbcTemplate.update(
            """
            INSERT INTO cliente (nombres, apellidos, rut, dvrut, email, contrasena, rol)
            VALUES ('Admin', 'Pedidos360', 11111111, '1', 'admin@pedidos360.cl', 'admin360', 'ADMIN')
        ...
        jdbcTemplate.update(
            """
            INSERT INTO cliente (nombres, apellidos, rut, dvrut, email, contrasena, rol)
            VALUES ('Operador', 'Pedidos360', 22222222, '2', 'operador@pedidos360.cl', 'operador360', 'OPERADOR')
            ON DUPLICATE KEY UPDATE rol = 'OPERADOR'
            """
        );
    }
