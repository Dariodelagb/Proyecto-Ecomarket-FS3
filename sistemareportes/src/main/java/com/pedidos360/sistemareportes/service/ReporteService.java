...
import com.pedidos360.sistemareportes.client.AuditoriaClient;
...
import com.pedidos360.sistemareportes.dto.BodegaDTO;
import com.pedidos360.sistemareportes.dto.ClienteDTO;
import com.pedidos360.sistemareportes.dto.DetalleVentaDTO;
...
public class ReporteService {
    ...
    public byte[] generarReporteExcel() {
        ...
        List<RegistroAuditoriaDTO> auditoria = auditoriaClient.obtenerRegistros(reportesInternalKey);
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            ...
            crearHojaAuditoria(workbook, styles, auditoria);
    ...
    private void crearHojaVentas(Workbook workbook, ReportStyles styles, List<VentaDTO> ventas) {
        ...
        for (VentaDTO venta : ventas) {
            ...
            crearCell(row, 5, texto(venta.getEstado()), styles.normal);
            crearCell(row, 6, productosVenta(venta), styles.normal);
            crearCell(row, 7, numero(venta.getMonto()), styles.money);
        }

        autoSize(sheet, 8);
    }

    private void crearHojaAuditoria(
        Workbook workbook,
        ReportStyles styles,
        ...
        int rowIndex = 1;
        for (RegistroAuditoriaDTO registro : registros) {
            Row row = sheet.createRow(rowIndex++);
            crearCell(row, 0, registro.getId(), styles.normal);
            crearCell(row, 1, registro.getFecha(), styles.dateTime);
            crearCell(row, 2, registro.getAccion(), styles.normal);
            crearCell(row, 3, registro.getEntidad(), styles.normal);
            crearCell(row, 4, registro.getEntidadId(), styles.normal);
            crearCell(row, 5, registro.getActorId(), styles.normal);
            crearCell(row, 6, registro.getActorNombre(), styles.normal);
            crearCell(row, 7, registro.getActorRol(), styles.normal);
            crearCell(row, 8, registro.getDetalle(), styles.normal);
        }
    ...
    private ReportStyles crearEstilos(Workbook workbook) {
        ...
        dateTime.setDataFormat(creationHelper.createDataFormat().getFormat("yyyy-mm-dd hh:mm"));

        return new ReportStyles(header, normal, money, date, dateTime);
    }
